...
function laneFor(epic: Epic, index: number) {
  const startMonth = epic.startDate
    ? new Date(epic.startDate + "T00:00:00Z").getUTCMonth()
    : (index % 4) + 6;
  const endMonth = epic.targetDate
    ? new Date(epic.targetDate + "T00:00:00Z").getUTCMonth()
    : startMonth + 1;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createEpic.mutateAsync({
        ...
        color: String(form.get("color") ?? "BLUE") as Epic["color"],
  ...
  return (
    <div className="overflow-hidden rounded-md border border-border bg-background shadow-sm">
      ...
      <div className="min-w-[780px]">
        <div className="grid grid-cols-[220px_repeat(6,minmax(90px,1fr))] border-b border-border bg-muted/45 text-[11px] font-semibold text-muted-foreground">
          <div className="px-4 py-3">Initiative</div>
          {months.map((month) => (
            <div className="border-l border-border px-3 py-3" key={month}>
              {month}
            </div>
          ))}
        </div>
        {epics.length === 0 ? (
          <p className="p-8 text-center text-sm text-muted-foreground">
            Create an epic to start mapping delivery work on the roadmap.
          </p>
        ) : (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {epic.targetDate ?? "No target date"}
                    </p>
                  </div>
                </div>
              ))}
            </div>
            <div className="relative divide-y divide-border bg-[linear-gradient(to_right,transparent_calc(16.66%-1px),#dfe1e6_calc(16.66%-1px),#dfe1e6_16.66%,transparent_16.66%)] bg-[length:16.66%_100%]">
              <div className="absolute inset-y-0 left-1/2 z-10 border-l-2 border-primary/70">
                <span className="absolute -top-0.5 -translate-x-1/2 bg-primary px-1.5 py-0.5 text-[9px] font-semibold text-white">
                  Today
                </span>
              </div>
              {epics.map((epic, index) => {
                const lane = laneFor(epic, index);

                return (
                  <div
                    className="grid h-20 grid-cols-6 items-center px-3"
                    key={epic.id}
                  >
                    <div
                      className={[
                        "h-7 rounded-sm px-2 py-1.5 text-[10px] font-semibold",
                        colors[epic.color],
                      ].join(" ")}
                      style={{
                        gridColumn:
                          String(lane.start) + " / span " + String(lane.span),
                      }}
                    >
                      {epic.name}
                    </div>
