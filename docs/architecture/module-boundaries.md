...
| IAM → others | Other services store IAM user UUIDs as external references; none reads `iam_db`. Project resolves member emails through IAM `POST /internal/users/lookup` ([ADR 0003](../decisions/0003-workspace-member-identity-lookup.md)). |
