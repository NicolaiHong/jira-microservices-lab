...
| `GET /api/projects/{projectId}/issues` | Workspace member | query `status?`, `assigneeUserId?`, `sprintId?`, `q?`, `limit?` (1–50, default 25), `cursor?` | `200 { items, nextCursor }` | project visible; invalid, unknown, or repeated parameter or malformed cursor → `400 VALIDATION_ERROR` | ISSUE-001, SEARCH-001 |
...
A PATCH with none of `summary`, `description`, `type`, or `priority` validates body shape and a supplied `expectedVersion` syntax only, performs visibility-only access, and returns the current Issue without CAS/version/history/outbox effects; it does not require an active Project solely for that no-op. For a transition, error precedence is body/status validation, Project visibility/writable state, expected-version validation/conflict, then graph legality. A successful transition atomically updates status/version once and writes exactly one `STATUS_CHANGED` history row plus one `issue.transitioned` outbox event. No transition idempotency-key contract is currently supported.

## Issue list

The project issue list is ordered by `(created_at, id)`, newest first, and is read one page at a time ([ADR 0005](../decisions/0005-issue-list-pagination-and-filtering.md)). Supplied filters combine with AND: `status` is `TODO|IN_PROGRESS|DONE` (case-insensitive), `assigneeUserId` and `sprintId` are UUIDs, and `q` is trimmed, 1–200 characters, and matched case-insensitively anywhere in `summary` with `%` and `_` taken literally. `nextCursor` is an opaque string to pass back unchanged as `cursor` with the same filters, or `null` on the last page. Following it until `null` returns every issue that existed when the first page was requested exactly once. Validation errors name the parameter in `details`. There is no sort parameter.
